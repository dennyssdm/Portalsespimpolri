...
export default async function Page() {
  const path = "/profil/tugas-fungsi"
  ...
  let dbContent = null
  try {
    const res = await serverFetch('/api/profil-content/p-3', { cache: 'no-store' })
    if (res.ok) {
    ...
    console.warn('Failed to fetch dynamic tugas-fungsi from DB, using fallback:', err)
  }
    ...
    if (dbContent.image_url !== undefined && dbContent.image_url !== null) {
      if (dbContent.image_url === '') {
        ...
        dynamicContent.media = {
          ...
          alt: dbContent.title || 'Foto Tugas Fungsi Sespim',
    ...
    if (dbContent.content) {
      ...
      for (const line of lines) {
        ...
        if (isHeader) {
          ...
          if (!currentSection) {
            currentSection = {
              title: dbContent.title || 'Tugas & Fungsi',
              body: '',
              items: []
            }
            sectionsList.push(currentSection)
