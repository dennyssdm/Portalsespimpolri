...
export default async function Page() {
  const categoryName = "Karya Peserta Didik"
  const path = "/publikasi/karya-peserta-didik"
  const eyebrow = "Publikasi"
  const title = "Karya Peserta Didik"
  const description = "Kumpulan karya tulis terbaik, esai, dan produk akademik yang disusun oleh para peserta didik Sespim."
  ...
  try {
    const res = await serverFetch('/api/publikasi-content/pub-karya-peserta-didik', { cache: 'no-store' })
    if (res.ok) {
    ...
    console.warn("Failed to fetch Karya Peserta Didik from API, using fallback:", err)
  }
