...
export default async function Page() {
  const categoryName = "Kajian Strategis"
  const path = "/publikasi/kajian-strategis"
  const eyebrow = "Publikasi"
  const title = "Kajian Strategis"
  const description = "Analisis mendalam mengenai isu-isu keamanan nasional, geopolitik, dan transformasi institusi Polri."
  ...
  try {
    const res = await serverFetch('/api/publikasi-content/pub-kajian-strategis', { cache: 'no-store' })
    if (res.ok) {
    ...
    console.warn("Failed to fetch Kajian Strategis from API, using fallback:", err)
  }
