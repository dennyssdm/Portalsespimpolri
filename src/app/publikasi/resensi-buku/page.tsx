...
export default async function Page() {
  const categoryName = "Resensi Buku"
  const path = "/publikasi/resensi-buku"
  const eyebrow = "Publikasi"
  const title = "Resensi Buku"
  const description = "Kritik, ulasan, dan analisis buku-buku referensi kepemimpinan dan manajemen organisasi kepolisian."
  ...
  try {
    const res = await serverFetch('/api/publikasi-content/pub-resensi-buku', { cache: 'no-store' })
    if (res.ok) {
    ...
    console.warn("Failed to fetch Resensi Buku from API, using fallback:", err)
  }
