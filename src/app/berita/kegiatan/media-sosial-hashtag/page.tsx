...
export default async function Page() {
  const path = "/berita/kegiatan/media-sosial-hashtag"
  ...
  let dbContent = null
  try {
    const res = await serverFetch('/api/berita-informasi-content/n-4', { cache: 'no-store' })
    if (res.ok) {
    ...
    console.warn('Failed to fetch dynamic content for /berita/kegiatan/media-sosial-hashtag:', err)
  }
