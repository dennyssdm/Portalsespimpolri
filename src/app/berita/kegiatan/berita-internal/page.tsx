...
export default async function Page() {
  const path = "/berita/kegiatan/berita-internal"
  ...
  let dbContent = null
  try {
    const res = await serverFetch('/api/berita-informasi-content/n-1', { cache: 'no-store' })
    if (res.ok) {
    ...
    console.warn('Failed to fetch dynamic content for /berita/kegiatan/berita-internal:', err)
  }
