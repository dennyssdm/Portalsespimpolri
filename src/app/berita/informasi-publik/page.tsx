...
export default async function Page() {
  const path = "/berita/informasi-publik"
  ...
  let dbContent = null
  try {
    const res = await serverFetch('/api/berita-informasi-content/n-6', { cache: 'no-store' })
    if (res.ok) {
    ...
    console.warn('Failed to fetch dynamic content for /berita/informasi-publik:', err)
  }
